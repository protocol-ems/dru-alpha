...
def save_stripe_info(request):
    ...
    if len(customer_data) == 0:
        customer = stripe.Customer.create(
            email=email, payment_method=payment_method_id, invoice_settings={'default_payment_method': payment_method_id}, phone=phone, name=name, address={'city': city, 'line1': line1, 'line2': line2, 'postal_code': postal_code, 'state': state})
    else:
    ...
    return Response(status=status.HTTP_200_OK,
                    data={'message': 'Success', 'customer_id': customer.id, 'extra_msg': extra_msg, 'sub info': subscription_info
                          })
    # return Response(status=status.HTTP_200_OK,
...
def customer_info(request):
    ...
    return Response(status=status.HTTP_200_OK, data={'customer info': customer_info})
...
def subscription_info(request):
    ...
    return Response(status=status.HTTP_200_OK, data={'subscription info': subscription_info})
